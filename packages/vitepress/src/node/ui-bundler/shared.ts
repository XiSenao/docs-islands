...
export function resolveSafeOutputPath(
  ...
  if (typeof fileName !== 'string' || fileName.trim().length === 0) {
    throw new TypeError(
      `[ui-bundler] Invalid output file name: ${String(fileName)}`,
    );
  }

  if (fileName.includes('\0')) {
    throw new Error(
      '[ui-bundler] Output file name must not contain null bytes',
    );
  }

  if (isAbsolute(fileName)) {
    throw new Error(
      `[ui-bundler] Absolute output paths are not allowed: ${fileName}`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `[ui-bundler] Refusing to write outside output directory. outDir="${normalizedOutDir}", fileName="${fileName}"`,
    );
  }
...
export function createComponentEntryModules({
  ...
  const entries = components.map((component) => {
    ...
    /**
     * Keep generated entry modules framework-neutral so the shared bundler does
     * not imply TSX/JSX semantics for future adapters such as Solid or Svelte.
     */
