...
import type { UIFrameworkBundlerAdapter } from './adapter';
...
const writeSpaSyncRenderedPageClientChunks = ({
  ...
  for (const [
    ...
    if (stats.totalTransformations > 0) {
      loggerInstance.getLoggerByGroup('ui-bundler-ssr-integration-processor')
        .success(`
    ...
    loggerInstance
      .getLoggerByGroup('ui-bundler-ssr-integration-processor')
      .info(
...
const collectPageComponentBundles = ({
  ...
  for (const el of elementsToRender.toArray()) {
    ...
    if (!importReference) {
      loggerInstance
        .getLoggerByGroup('ui-bundler-transform-html')
        .warn(
...
export interface RegisterUIFrameworkBuildHooksOptions {
  adapter: UIFrameworkBundlerAdapter;
...
export function registerUIFrameworkBuildHooks(
  ...
  vitepressConfig.transformHtml = async (html, id, ctx) => {
    ...
    const Logger = loggerInstance.getLoggerByGroup('ui-bundler-transform-html');
  ...
  vitepressConfig.buildEnd = async () => {
    const { assetsDir, cacheDir, cleanUrls, outDir, root, siteDebug, srcDir } =
      config;
    const metafileDir = join(outDir, assetsDir);
    const Logger = loggerInstance.getLoggerByGroup('ui-bundler-build-end');
