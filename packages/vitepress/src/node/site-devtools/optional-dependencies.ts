import { pkgExists } from '@docs-islands/utils/general';
...
export function resolveSiteDevToolsOptionalDependencyFallbackPath({
  builtRelativePath,
...
const siteDevtoolsOptionalDependencyFallbacks = [
  {
    dependency: 'vue-json-pretty',
    specifiers: [
      {
        find: 'vue-json-pretty',
        replacement: resolveSiteDevToolsOptionalDependencyFallbackPath({
...
export function applySiteDevToolsOptionalDependencyFallbacks(
  vitepressConfig: UserConfig<DefaultTheme.Config>,
  resolutionBase: string,
): void {
  const missingAliases = siteDevtoolsOptionalDependencyFallbacks.flatMap(
