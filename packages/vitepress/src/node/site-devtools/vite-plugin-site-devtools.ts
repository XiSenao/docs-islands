...
import { resolveSiteDevToolsPath } from './shared';

export interface SiteDevToolsVitePluginOptions {
  base: string;
  enabled: boolean;
  pluginName: string;
}

export function getSiteDevToolsVitePlugins({
  base,
  enabled,
  pluginName,
}: SiteDevToolsVitePluginOptions): PluginOption[] {
  if (!enabled) {
    return [];
  }
  ...
  return [
    {
      ...
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          ...
          const debugSourcePath = resolveSiteDevToolsPath(
            base,
