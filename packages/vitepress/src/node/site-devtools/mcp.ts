...
import type {
  ...
  SiteDevToolsBuildDataOptions,
  SiteDevToolsBuildOverview,
  SiteDevToolsBuildPageSummary,
  SiteDevToolsBuildReportPayload,
} from './build-data';
import { SiteDevToolsBuildDataStore } from './build-data';
...
interface SiteDevToolsBuildMcpResource {
  description: string;
...
interface SiteDevToolsBuildMcpToolStringSchema {
  enum?: readonly string[];
  type: 'string';
}

interface SiteDevToolsBuildMcpToolInputSchema {
  additionalProperties: false;
  properties: Record<string, SiteDevToolsBuildMcpToolStringSchema>;
  required?: readonly string[];
  type: 'object';
}

type SiteDevToolsBuildToolArguments = Record<string, string | undefined>;

interface SiteDevToolsBuildMcpToolDefinition {
  description: string;
  execute: (
    dataStore: SiteDevToolsBuildDataStore,
    args: SiteDevToolsBuildToolArguments,
  ) => Promise<SiteDevToolsBuildMcpToolResult> | SiteDevToolsBuildMcpToolResult;
  inputSchema: SiteDevToolsBuildMcpToolInputSchema;
  name: SiteDevToolsBuildMcpToolName;
  validate?: (args: SiteDevToolsBuildToolArguments) => void;
}
...
export type SiteDevToolsBuildMcpToolName =
  | 'get_artifact'
  | 'get_build_overview'
  | 'get_build_report'
  | 'get_component'
...
export interface SiteDevToolsBuildMcpServerOptions
  extends SiteDevToolsBuildDataOptions {
  stderr?: Writable;
  stdin?: NodeJS.ReadableStream;
  stdout?: Writable;
}

type SiteDevToolsBuildMcpToolResult =
  | SiteDevToolsBuildArtifactSnapshot
  | SiteDevToolsBuildComponentSummary
  | {
      error: string;
...
const SERVER_NAME = 'docs-islands-site-devtools-build-mcp';
const SERVER_VERSION = '1.0.0';
const PROTOCOL_VERSION = '2024-11-05';
const HELP_TEXT = `
Usage:
  docs-islands-vitepress-site-devtools-mcp --outDir <path> [--assetsDir <dir>]
...
const createToolInputSchema = ({
  ...
  properties: SiteDevToolsBuildMcpToolInputSchema['properties'];
  required?: readonly string[];
}): SiteDevToolsBuildMcpToolInputSchema => ({
  additionalProperties: false,
...
const MCP_RESOURCES: SiteDevToolsBuildMcpResource[] = [
  {
    description:
      'Overview of the build-time data exposed by the Site DevTools MCP server.',
    mimeType: 'text/markdown',
    name: 'site-devtools-build-data-overview',
    text: [
      '# Site DevTools Build Data Overview',
      '',
      'This MCP server exposes build-time data generated by docs-islands Site DevTools instrumentation.',
      '',
      'Available data:',
    ...
    uri: 'file://docs-islands/site-devtools-build-data-overview.md',
  },
  {
    description:
      'Recommended tool usage flow for exploring docs-islands build artifacts.',
    mimeType: 'text/markdown',
    name: 'site-devtools-build-tool-guide',
    text: [
      '# Site DevTools Build Tool Guide',
      '',
...
const MCP_TOOL_DEFINITIONS = [
  {
    ...
    execute: (dataStore: SiteDevToolsBuildDataStore) =>
  ...
  },
  {
    ...
    execute: (dataStore: SiteDevToolsBuildDataStore) => dataStore.listPages(),
    inputSchema: createToolInputSchema({
      properties: {},
    }),
    name: 'list_pages',
  ...
  },
  {
    ...
    execute: (
      ...
      dataStore.getArtifact({
        artifactKind:
          args.artifactKind as SiteDevToolsBuildArtifactSnapshot['artifactKind'],
    ...
    name: 'get_artifact',
    validate: (args: SiteDevToolsBuildToolArguments) => {
...
] as const satisfies readonly SiteDevToolsBuildMcpToolDefinition[];
...
const serializeToolResult = (
  result: SiteDevToolsBuildMcpToolResult,
  isError = false,
) => ({
  content: [
    {
...
const validateToolArguments = ({
  ...
  args: unknown;
  schema: SiteDevToolsBuildMcpToolInputSchema;
  toolName: SiteDevToolsBuildMcpToolName;
}): SiteDevToolsBuildToolArguments => {
  ...
  const normalizedArgs: SiteDevToolsBuildToolArguments = {};
...
const parseCliArgs = (argv: string[]): SiteDevToolsBuildDataOptions | null => {
  const nextOptions: SiteDevToolsBuildDataOptions = {
    outDir: '',
  };
...
export interface SiteDevToolsBuildMcpServer {
  close: () => void;
  handleMessage: (message: unknown) => Promise<JsonRpcResponse | null>;
  listen: () => void;
}

export const createSiteDevToolsBuildMcpServer = (
  options: SiteDevToolsBuildMcpServerOptions,
): SiteDevToolsBuildMcpServer => {
  const dataStore = new SiteDevToolsBuildDataStore(options);
  ...
  const handleToolCall = async (
    tool: SiteDevToolsBuildMcpToolDefinition,
    args: unknown,
  ) => {
    try {
...
export const startSiteDevToolsBuildMcpServer = (
  options: SiteDevToolsBuildMcpServerOptions,
): SiteDevToolsBuildMcpServer => {
  const server = createSiteDevToolsBuildMcpServer(options);
  server.listen();
  return server;
};

export const runSiteDevToolsBuildMcpCli = async (
  ...
  try {
    ...
    startSiteDevToolsBuildMcpServer(options);
  } catch (error) {
...
if (process.argv[1] && process.argv[1] === currentFilePath) {
  runSiteDevToolsBuildMcpCli().catch((error) => {
    process.stderr.write(
      `${error instanceof Error ? error.message : 'Failed to start MCP server.'}\n`,
    );
    process.exitCode = 1;
