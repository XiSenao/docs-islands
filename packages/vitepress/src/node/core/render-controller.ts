import type { PageMetafile, UsedSnippetContainerType } from '@docs-islands/vitepress-types';
...
export class RenderController {
  ...
  public deleteCompilationContainerByMarkdownModuleId(markdownModuleId: string): void {
    this.#markdownModuleIdToCompilationContainerMap.delete(markdownModuleId);
  }

  public hasCompilationContainerByMarkdownModuleId(markdownModuleId: string): boolean {
    return this.#markdownModuleIdToCompilationContainerMap.has(markdownModuleId);
  }

  public getCompilationContainerByMarkdownModuleId(
    markdownModuleId: string
  ): CompilationContainerType | Promise<CompilationContainerType> {
    if (!this.#markdownModuleIdToCompilationContainerMap.has(markdownModuleId)) {
      const pendingPromise = new Promise<CompilationContainerType>(resolve => {
        this.markdownModuleIdToPendingResolvedCompilationContainerMap.set(
          markdownModuleId,
          resolve
        );
      });
      this.#markdownModuleIdToCompilationContainerMap.set(markdownModuleId, pendingPromise);
      return pendingPromise;
    }

    return this.#markdownModuleIdToCompilationContainerMap.get(markdownModuleId)!;
  }

  public async getComponentFullPathToPageIdAndImportedNameMap(): Promise<{
    ssrOnlyComponentFullPathToPageIdAndImportedNameMap: Map<string, Set<string>>;
    nonSSROnlyComponentFullPathToPageIdAndImportedNameMap: Map<string, Set<string>>;
  }> {
    const ssrOnlyComponentFullPathToPageIdAndImportedNameMap = new Map<string, Set<string>>();
    const nonSSROnlyComponentFullPathToPageIdAndImportedNameMap = new Map<string, Set<string>>();
    for (const [markdownModuleId, compilationContainer] of this
      ...
      const compilationContainerResult: CompilationContainerType =
        compilationContainer instanceof Promise ? await compilationContainer : compilationContainer;

      const { ssrOnlyComponentNames, importsByLocalName } = compilationContainerResult;
      if (ssrOnlyComponentNames) {
        for (const componentName of ssrOnlyComponentNames) {
          ...
          if (importInfo) {
            const pageIds =
              ssrOnlyComponentFullPathToPageIdAndImportedNameMap.get(importInfo.identifier) ||
              new Set<string>();
            pageIds.add(`${markdownModuleId}__SSR_ONLY_PLACEHOLDER__${componentName}`);
            ssrOnlyComponentFullPathToPageIdAndImportedNameMap.set(importInfo.identifier, pageIds);
          }
        }
      }
      ...
      if (importsByLocalName) {
        for (const [localName, importInfo] of importsByLocalName.entries()) {
          ...
          const pageIds =
            nonSSROnlyComponentFullPathToPageIdAndImportedNameMap.get(importInfo.identifier) ||
            new Set<string>();
          pageIds.add(`${markdownModuleId}__NON_SSR_ONLY_PLACEHOLDER__${localName}`);
          nonSSROnlyComponentFullPathToPageIdAndImportedNameMap.set(importInfo.identifier, pageIds);
        }
      }
    }
  ...
  public setCompilationContainer(
    ...
    this.#markdownModuleIdToCompilationContainerMap.set(markdownModuleId, compilationContainer);
  }
  ...
  public getUsedSnippetContainerByMarkdownModuleId(
    ...
    return this.#markdownModuleIdToUsedSnippetContainerMap.get(markdownModuleId);
  }
  ...
  public setUsedSnippetContainer(
    ...
    this.#markdownModuleIdToUsedSnippetContainerMap.set(markdownModuleId, usedSnippetContainer);
  }
  ...
  public getTransformedPageMetafile(): Record<string, PageMetafile> {
    ...
    for (const [page, pageMetafile] of this.#pageToPageMetafileMap.entries()) {
      transformedPageMetafileMap[`/${page.replace(/\.md$/, '')}`] = pageMetafile;
    }
