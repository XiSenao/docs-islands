...
`@docs-islands/vitepress` 为 `VitePress` 带来跨框架孤岛组件渲染能力，让你可以在 Markdown 中直接接入孤岛组件，并按组件粒度选择渲染策略，而不必放弃 `VitePress` 原本的静态优先体验。
...
- 在 Markdown 中直接复用现有孤岛组件。
- 对不同交互块分别决定是纯静态、立即水合、可见时水合，还是纯客户端渲染。
- 在 `SPA` 切页时尽量减少孤岛组件的闪烁与 CLS 抖动。
...
- 框架隔离：每个孤岛组件都运行在各自独立的渲染容器里，避免全局混杂。
...
- 你的页面主体本身就是一个完整的重交互应用，而不是“文档里插入少量孤岛组件”。
- 组件之间依赖复杂的跨框架共享状态，希望多个框架之间天然保持响应式双向通信。
- 你希望它替代现有前端应用框架的页面级路由、数据流和应用壳模型。

## 下一步

- 从 [快速上手](./getting-started.md) 开始接入。
- 在 [工作原理](./how-it-works.md) 里理解注入链路、运行阶段、策略行为和 `spa:sync-render`。
- 在 [最佳实践](./best-practices.md) 里查看写法规则、策略选择建议和注意事项。
