...
import type { SiteDevToolsWindow } from './site-devtools-shared';
...
export const getCurrentPageCandidates = (
  debugWindow: SiteDevToolsWindow,
...
export const getSiteBasePath = (debugWindow: SiteDevToolsWindow) => {
  const base = debugWindow.__VP_SITE_DATA__?.base;

  return typeof base === 'string' && base.length > 0 ? base : '/';
};

export const isSiteDevToolsAiReportRuntimeAvailable = (isDev: boolean) =>
  !isDev;
...
export const getRenderMetricKey = (metric: SiteDevToolsRenderMetric) =>
