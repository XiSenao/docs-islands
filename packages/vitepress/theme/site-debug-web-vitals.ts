...
import {
  ...
  type SiteDebugRenderMetricWebVitalsAnalysis,
  type SiteDebugWebVitalsLayoutShift,
  type SiteDebugWebVitalsLongTask,
  type SiteDebugWebVitalsSnapshot,
} from '../src/shared/site-debug-web-vitals-analysis';
...
export const SITE_DEBUG_WEB_VITALS_EVENT_NAME =
  'docs-islands:site-debug-web-vitals';

const webVitalsSnapshot: SiteDebugWebVitalsSnapshot = {};
const longTaskEntries: SiteDebugWebVitalsLongTask[] = [];
const layoutShiftEntries: SiteDebugWebVitalsLayoutShift[] = [];
...
const scheduleVitalsUpdate = () => {
  ...
  notifyTimer = globalThis.setTimeout(() => {
    ...
    globalThis.dispatchEvent(new CustomEvent(SITE_DEBUG_WEB_VITALS_EVENT_NAME));
  }, 80);
};
...
export const ensureSiteDebugWebVitalsTracking = () => {
...
export const destroySiteDebugWebVitalsTracking = () => {
...
export const getSiteDebugWebVitalsSnapshot = () =>
  Object.keys(webVitalsSnapshot).length > 0 ? { ...webVitalsSnapshot } : null;

export const analyzeSiteDebugRenderMetricWebVitals = (
  ...
  return analyzeRenderMetricWebVitals(metric, {
    ...
    snapshot: getSiteDebugWebVitalsSnapshot(),
  });
};

export type {
  SiteDebugRenderMetricWebVitalsAnalysis,
  SiteDebugWebVitalsSnapshot,
} from '../src/shared/site-debug-web-vitals-analysis';
