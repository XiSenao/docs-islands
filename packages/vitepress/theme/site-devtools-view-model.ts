import type { SiteDevToolsWebVitalRating } from '#shared/site-devtools-web-vitals-analysis';
import type {
...
export const getWebVitalRatingLabel = (rating?: SiteDevToolsWebVitalRating) => {
  switch (rating) {
    case 'good': {
      return 'Good';
    }
...
export const getWebVitalRatingTone = (rating?: SiteDevToolsWebVitalRating) => {
  switch (rating) {
    case 'good': {
      return 'is-success';
    }
...
export const getWebVitalsContextLabel = (
  analysis: SiteDevToolsRenderMetricWebVitalsAnalysis,
) =>
...
export const getHmrStageItems = (metric: SiteDevToolsHmrMetric) =>
  [
    {
...
export const getHmrEventItems = (metric: SiteDevToolsHmrMetric) =>
  [
    {
      label: 'Trigger',
...
export const getEqualGridStyle = (itemCount: number) => ({
  '--site-devtools-grid-columns': String(Math.max(itemCount, 1)),
});
...
export const getTotalDurationBreakdown = (metric: SiteDevToolsRenderMetric) => {
  const parts: {
    description: string;
    key: string;
    label: string;
...
export const getWebVitalsDetailItems = (
  analysis: SiteDevToolsRenderMetricWebVitalsAnalysis | null,
) => {
  if (!analysis) {
    return [];
  }
