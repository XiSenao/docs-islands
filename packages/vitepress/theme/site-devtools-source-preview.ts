import { formatBytes, inferSourceLanguage } from './site-devtools-shared';
import {
...
} from './site-devtools-source-text-index';
...
const BACKGROUND_PREVIEW_ABORT_ERROR = 'SiteDevToolsBackgroundPreviewAborted';
...
export const highlightCodeContent = async (
  ...
  try {
    ...
    return `<pre class="shiki site-devtools-source-viewer__fallback"><code>${escapeHtml(sourceContent)}</code></pre>`;
  }
};
...
export const createBackgroundCodePreviewRenderer = () => {
  ...
  const ensureWorker = () => {
    ...
    worker = new Worker(
      new URL('site-devtools-source-preview.worker.ts', import.meta.url),
      {
        type: 'module',
      },
    );
...
export const createBackgroundWindowedCodePreviewHighlighter = () => {
  ...
  const ensureWorker = () => {
    ...
    worker = new Worker(
      new URL('site-devtools-source-highlight.worker.ts', import.meta.url),
      {
        type: 'module',
      },
    );
...
export const createBackgroundPlainTextPreviewIndexer = () => {
  ...
  const ensureWorker = () => {
    ...
    // runtime-relative URL that points at a missing file.
    worker = new Worker(
      new URL('site-devtools-source-text.worker.ts', import.meta.url),
      {
        type: 'module',
      },
    );
