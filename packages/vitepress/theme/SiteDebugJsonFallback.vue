...
<template>
  <div class="site-debug-json-fallback" :style="containerStyle">
    <p class="site-debug-json-fallback__label">{{ rootPath }}</p>
    <pre
      class="site-debug-json-fallback__content"
    ><code>{{ serializedValue }}</code></pre>
  </div>
</template>

<style scoped>
.site-debug-json-fallback {
  overflow: auto;
...
.site-debug-json-fallback__label {
  margin: 0;
  padding: 10px 14px;
...
.site-debug-json-fallback__content {
  margin: 0;
  padding: 14px;
  min-height: 100%;
  font-size: 12px;
