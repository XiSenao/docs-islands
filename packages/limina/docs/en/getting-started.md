...
Checker identities are `tsc`, `tsgo`, `vue-tsc`, `astro`, and `svelte-check`; every managed type config finishes with exactly one of them. The first three can emit declarations, while Astro and Svelte execute per leaf without declaration projection. Install the matching package when an owner is active; `tsgo` requires `@typescript/native-preview`. Astro checks require `astro`, `@astrojs/check`, and `typescript` in the owning leaf; Svelte checks and semantic graph analysis require `svelte-check`, `svelte2tsx`, `svelte`, and `typescript` in that leaf. Lightweight Astro import collection requires one workspace-level `@astrojs/compiler` installation beside Limina. Eligible graph imports then use the supported owner-scoped Astro/check semantic tuple described in [Checker Configuration](./config/checkers.md); pnpm store or hoist paths may differ and are not compatibility conditions. Vue source evidence needs no separate parser package. Vue semantic graph analysis resolves a supported `vue-tsc` from the checker execution scope and its internal toolchain from that checker installation; applications do not install Language Core or Volar TypeScript for Limina. Limina does not directly require `@vue/compiler-sfc`.
