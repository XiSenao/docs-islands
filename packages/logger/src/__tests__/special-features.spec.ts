    ...
    it('Case 26: Success level in rule mode', () => {
      ...
      expect(consoleLogSpy).toHaveBeenNthCalledWith(
        ...
        expect.stringMatching(
          /\[Test1\].*@docs-islands\/test\[test\.success\.default\]: task done.*\d\.\d+ms$/,
        ),
      );
      expect(consoleLogSpy).toHaveBeenNthCalledWith(
        2,
        expect.stringMatching(
          /\[Test2\].*@docs-islands\/test\[test\.success\.other\]: job completed.*\d\.\d+ms$/,
        ),
      );
    });
  });
  ...
  describe('Picomatch Operators', () => {
    it('Case 27: Picomatch operators ? and []', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenNthCalledWith(
        1,
        expect.stringMatching(
          /\[Test1\].*@docs-islands\/test\[test\.case\.a1\]: noop.*\d\.\d+ms$/,
        ),
      );
      expect(consoleErrorSpy).toHaveBeenNthCalledWith(
        1,
        expect.stringMatching(
          /\[Test2\].*@docs-islands\/test\[test\.case\.a1\]: task-a.*\d\.\d+ms$/,
        ),
      );
      expect(consoleErrorSpy).toHaveBeenNthCalledWith(
        2,
        expect.stringMatching(
          /\[Test2\].*@docs-islands\/test\[test\.case\.ab1\]: task-b.*\d\.\d+ms$/,
        ),
      );
    });
  });
