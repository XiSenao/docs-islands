...
`SourceEvidence` retains standalone syntax, diagnostics, and source coordinates. It is not checker, reference, dependency-graph, graph-check, or source-check semantic authority. The production provider admits only `direct-source` and strict `mapped-source` `ProjectDependency` variants. A generated dependency without one source-authored provenance becomes an `unmapped-generated` observation even if it resolves to governed workspace source; it cannot become a source-derived edge. Ambiguous reverse mapping is a structured failure, not an ignorable observation.

Source inspection uses ownership-scoped parser providers only to recover source-authored records and coordinates. Astro source inspection resolves and initializes the Limina-owned `@astrojs/compiler` from the workspace installation running Limina; one shared runtime failure has one stable issue identity rather than one issue per `.astro` file. Svelte source-range inspection resolves the public `svelte/compiler` entry from the owning leaf. Vue source inspection has no configurable backend and covers inline scripts, `<script src>`, and `generic` attribute `import()` expressions without initializing `vue-tsc`. A Vue-semantic source is classified by its semantic project identity as `vue-sfc`, `vitepress-markdown`, or `petite-vue-html` according to Language Core plugin precedence. The `vitepress-markdown` profile masks inline and fenced backtick code ranges with offset-preserving spaces before source-record collection; tilde fences remain visible to match the shared Language Core 2.2 and 3.2 behavior.
...
Vue graph semantics are produced lazily through one internal context containing Language Core, the Volar language service host, TypeScript Program, source maps, and the host module-resolution cache. A manager retains at most one live heavy Vue context globally, leases it by project batch, disposes it deterministically when switching identity, and versions it with the identity generation. For every framework source, the provider materializes the service script and enumerates its dependencies with the Vue toolchain TypeScript AST before reverse mapping. Only one strict source mapping can create a project dependency. `sourceSpecifier` and `semanticSpecifier` are distinct views, so `<script src="./entry.ts">` may resolve through a generated `./entry.js` literal. Synthetic service-script imports remain observations; absent or ambiguous mappings fail closed and do not become edges. Resolution uses the mapped literal and Volar-aware host resolver with no Oxc fallback. TypeScript files inside a Vue-semantic project use the direct TypeScript sub-semantics. Owner lookup, declaration-provider selection, checker compatibility, and reference recording remain Limina-owned policy shared by generated graph preparation and `graph:check`.

A parsed import whose consumer has no declaration project remains a `framework-schedule` edge even when its target is build-owned source. CSS, image, virtual, and other resource imports do not create project references. Only canonical relations between build-capable configs participate in declaration-component coloring and generated declaration references.
...
Svelte graph semantics use a bounded public adapter. The owning leaf supplies `svelte/compiler` for instance- and module-script source ranges and the supported optional `svelte2tsx` peer for generated TSX and its source map; users install `svelte2tsx` alongside `svelte-check`. Limina resolves both from that leaf only when Svelte semantic analysis is active, and neither publishes `svelte2tsx` as a production dependency nor bundles it into the CLI. The generated representation is enumerated with the TypeScript AST and reverse-mapped with the internally bundled `@jridgewell/trace-mapping` implementation before TypeScript-compatible module resolution. The adapter does not execute user preprocessors and does not import private `svelte-check` bundles. Synthetic generated dependencies remain observations, and missing or ambiguous provenance or toolchain failure fails closed with no Oxc fallback. Ordinary TypeScript files in a Svelte-semantic project use direct TypeScript sub-semantics.
...
`oxc-resolver` remains available for source-only runtime inspection and the narrowly bounded pending-ownership bootstrap. In ownership discovery it is called only after TypeScript type evidence is missing and can produce only a qualified physical framework candidate; it cannot produce `ProjectDependency`, checker resolution, graph evidence, a declaration provider, or a source-check semantic result. Once semantic authority is locked, the checker-semantic resolver always records `oxcResolvedFilePath: null`; a checker miss stays missing and a semantic failure is never rescued. Limina does not declare or load `oxc-parser`: source-authored and generated dependency enumeration use TypeScript syntax ASTs and scanners. Resource existence checks use filesystem and Node package-export resolution rather than treating Oxc as project authority.
