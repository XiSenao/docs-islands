...
When the field is omitted, or when its mode is `auto`, generated-graph preparation discovers source configuration scopes from the validated workspace context. Ordinary TypeScript initially belongs to `tsc`, or to `tsgo` when `useTsgo: true`; Vue belongs to `vue-tsc`. Dependency promotion can move a TypeScript consumer into the Vue-owned scope. Vue custom extensions are discovered in two stages: configuration and traversal evidence select a Vue parser candidate, but only the project-scoped Vue semantic identity's finalized file set confirms capability. That identity resolves `vue-tsc`, Vue Language Core, Volar TypeScript, and TypeScript once from the source config's dependency tree, then uses the same TypeScript runtime, config closure, virtual overlay, Vue options, project references, and file set for membership and semantic context creation. A Vue hint without a matching module does not switch ownership.
...
Framework import analysis uses project-scoped parser providers. Astro analysis resolves and initializes leaf-local `@astrojs/compiler` before auto dependency promotion; Svelte analysis resolves `svelte/compiler`. Vue lightweight source collection has no configurable backend and covers inline scripts, `<script src>`, and `generic` attribute `import()` expressions without initializing `vue-tsc`. A Vue-owned source is classified by the finalized project identity as `vue-sfc`, `vitepress-markdown`, or `petite-vue-html` according to Language Core plugin precedence. The `vitepress-markdown` profile masks inline and fenced backtick code ranges with offset-preserving spaces before lightweight collection; tilde fences remain visible to match the shared Language Core 2.2 and 3.2 behavior.
