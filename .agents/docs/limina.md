...
Pending ownership discovery uses one bounded native TypeScript semantic context for the parsed project, Program/AST dependency enumeration, module resolution, and ambient TypeChecker evidence. Its inclusion and resolution ledgers are occurrence-aware and kind-aware: module-like syntax uses the Program's per-occurrence resolution mode and redirected project reference, triple-slash paths use `referencedFiles` plus `resolveTripleslashReference`, type directives use TypeScript's type-reference channel, and libraries use TypeScript's library environment. Parsed roots, raw project references, explicit path/type references, libraries, and real external packages outside the current-run workspace source boundary are admitted for their TypeScript inclusion reason; an ordinary workspace module target that is not otherwise admitted remains a physical resolution result but cannot enter the Program. The boundary is only a normalized and realpath-aware Boolean set derived from the complete governed source roots; it carries no owner, reference, provenance, or ambient-policy relation and never performs resolution. Raw references are always read from the user source/resolver config; inferred declaration edges and generated-config references never feed this semantic context. Importer enumeration still visits only the explicit effective roots, never the Program closure. Source validation evaluates package and ambient rules independently after the semantic relation exists. Ambient and concrete declarations terminate at their typed boundary; checker source uses the checker-resolved target; unsupported semantic evidence fails closed. Only a missing result may invoke Oxc, whose result is qualified solely as a physical framework candidate. Admission requires a known framework extension, governed effective membership, one owning config, and one framework semantic domain. Ordinary TypeScript and resource targets never color ownership. Limina collects the complete requirement set before applying any dependency-derived lock, so import order cannot hide a cross-framework conflict.
...
Standalone `SourceEvidence` exists only for native JavaScript and TypeScript files; `.vue`, `.astro`, and `.svelte` require project/checker context. Locked native dependency collection uses the same kind-aware TypeScript resolution channels as pending ownership, so a triple-slash or mode-sensitive occurrence is not reinterpreted by a second generic module resolver. Its dependency-facts context admits only the complete effective root set into the Program while retaining TypeScript's raw semantic result for every root occurrence; standard libraries, project-reference files, explicit directive targets, type packages, and external package closure do not enter that Program. The root records and resolutions are copied into a generation-scoped snapshot before the Program is disposed. This snapshot is dependency evidence only and never substitutes for the full bounded Program used by pending ownership and `TypeEvidence`. Framework preparation directly produces `PreparedDependencyFact` values containing the source-located `ImportRecord`, generated `semanticSpecifier`, resolution mode, checker target, and existing `TypeEvidence`. `ProjectDependencyProvider` classifies those facts without calling framework resolution again. A generated dependency without source provenance becomes an `unmapped-generated` observation even if it could resolve to governed source; mapping damage, ambiguity, or semantic disagreement fails closed.
...
Checker project-config parsing and project-dependency caches belong to an `AnalysisProviderSet` and therefore to one repository generation. Graph, source, proof, owner, and checker projections share that generation's caches; advancing creates a new provider set and fresh caches. Project-dependency cache identities are compact hashes of canonical semantic inputs. Boundary-independent root syntax and TypeScript resolution snapshots can be reused inside the generation, while final dependency collections remain separated by the complete project semantic identity and any explicitly identified workspace-export policy. A resolver callback without a cache identity disables final collection caching. Direct parser calls without a cache remain uncached, and virtual-file identities remain separate from physical-file identities.
