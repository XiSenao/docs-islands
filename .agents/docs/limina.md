...
Local evidence and solution constraints are distinct. Config features, effective root files, dependency requirements, and directed Vue promotion can resolve a leaf's local owner. Declared solution owners and resolved leaf owners seed a fixed point over the solution/leaf bipartite graph, including overlapping solutions connected by shared pending leaves. Constraint propagation never writes a pending local owner. Compatibility is validated after every phase, and final solution leaf owners must be identical.

Dependency requirement analysis scans every managed type config, including resolved owners, and reduces all import requirements together. Pending projects and Astro/Svelte-owned TS/JS roots use neutral TypeScript TypeEvidence. Ambient, concrete-declaration, and checker-source evidence establishes a domain boundary; only missing evidence proceeds to canonical physical resolution and actual governed-file membership. A requirement can resolve a pending owner but can only validate a resolved owner; an incompatible requirement never overwrites it.

Vue build promotion follows dependency direction only: a pending consumer can inherit `vue-tsc` from a Vue provider. It does not color an undirected component or overwrite a resolved consumer. Ordinary pending configs fall back to `tsgo` only when `auto.useTsgo` is true, otherwise to `tsc`.
...
Framework import analysis uses ownership-scoped parser providers. Astro analysis resolves and initializes the Limina-owned `@astrojs/compiler` from the workspace installation running Limina before auto dependency promotion; one shared runtime failure has one stable issue identity rather than one issue per `.astro` file. Svelte analysis continues to resolve `svelte/compiler` from the owning leaf. Vue lightweight source collection has no configurable backend and covers inline scripts, `<script src>`, and `generic` attribute `import()` expressions without initializing `vue-tsc`. A Vue-owned source is classified by the finalized project identity as `vue-sfc`, `vitepress-markdown`, or `petite-vue-html` according to Language Core plugin precedence. The `vitepress-markdown` profile masks inline and fenced backtick code ranges with offset-preserving spaces before lightweight collection; tilde fences remain visible to match the shared Language Core 2.2 and 3.2 behavior.

Astro graph semantics use a separate bounded pipeline: a real lightweight `ImportRecord` plus cheap Oxc/specifier evidence first decides semantic eligibility; only eligible records may materialize an Astro context; final runtime classification runs afterward with the accumulated Oxc and TypeScript evidence. The seed contains existing config/project identity, analysis generation, overlay generation, and the owning package root, while config closure fingerprints, compiler options, files, project references, and toolchain provenance enter only the lazy materialized identity. The context registers Astro, Vue, and Svelte language plugins and creates a Volar Language with its decorated TypeScript host, but never a Language Service or Program. One analysis provider retains at most one active Astro context, reuses it by seed and toolchain identity, and disposes it before switching identity.

The first Astro semantic adapter accepts Astro `>=7.0.0 <8.0.0`, `@astrojs/check` 0.9.10, Language Server 2.16.13, LS-owned compiler 2.13.1, and Volar Language Core, Kit, and TypeScript 2.4.28. Leaf-visible and check-visible TypeScript are each checked against Limina's declared `>=5.4.0 <5.10.0 || >=6.0.0 <6.1.0` range; they need not have equal versions or real paths. Internal exports and callable shapes remain part of compatibility. Resolution follows declared ownership scopes from leaf to check to Language Server to Kit, with no workspace-root retry. Resolved paths are provenance and materialized module-instance identity only: pnpm store, symlink, and hoist layouts are never compatibility predicates and never enter stable issue identity.

Astro semantic mapping starts from the complete locator of a real source record and looks forward through primary and extra service scripts. Synthetic virtual imports may exist normally, but they are never candidates and never create graph edges. Missing, ambiguous, or unprovable mapping for a real record is a source-map mismatch; an unresolved target with a valid mapping remains an ordinary module-not-found result. All record-aware consumers reuse the canonical evidence, so each record and context generation performs at most one semantic host resolution. `.astro -> .astro` and `.astro -> .svelte` produce framework scheduling; `.astro -> .vue` uses Astro only to confirm the physical Vue target before the existing Vue-owned declaration/checker provider policy; `.astro -> .ts/.tsx` uses the TypeScript declaration provider. In `A.astro -> B.vue -> C.ts`, Astro owns `A -> B`, while Vue owns `B -> C` only when `B.vue` is analyzed independently.

Vue graph semantics are enriched lazily through one internal context containing Language Core, the Volar language service host, TypeScript Program, source maps, and the host module-resolution cache. A manager retains at most one live heavy Vue context globally, leases it by project batch, disposes it deterministically when switching identity, and versions it with the identity generation. Vue semantic dependencies must map from one raw source record to a strict virtual literal. Candidate mappings may merge only when semantic specifier, resolution mode, and normalized target agree; synthetic service-script imports and absent or ambiguous mappings fail closed and do not become edges. TypeScript/declaration resolution uses the mapped literal and Volar-aware host resolver, while Oxc/runtime and resource evidence continues to use the original source specifier. Owner lookup, declaration-provider selection, checker compatibility, and reference recording remain Limina-owned policy shared by generated graph preparation and `graph:check`.

A parsed framework import that resolves to declaration-compatible governed source is passed through the checker-aware declaration-provider policy and can add a generated declaration reference. Imports of pure framework modules remain scheduling relationships between wrapper or transparent solutions. CSS, image, virtual, and other resource imports do not create project references.

The Vue semantic adapter accepts only the explicitly tested families: `vue-tsc`/Language Core 2.2.0–2.2.12 with Volar TypeScript 2.4.11–2.4.28, or 3.2.0–3.2.4 with Volar TypeScript 2.4.27; both accept TypeScript 5.4.x–5.9.x or 6.0.x. The published manifest declares only `vue-tsc` as the optional Vue checker peer while TypeScript remains a required Limina runtime peer; Language Core and Volar TypeScript stay internal to the checker toolchain. The runtime resolves `vue-tsc` from its checker execution scope, then resolves all three internal toolchain packages, including the Vue toolchain's TypeScript, from that `vue-tsc` dependency environment and treats the complete tuple as compatibility authority. Missing or unsupported external checkers are diagnosed separately from an unsupported internal toolchain, whose fix always targets `vue-tsc` rather than direct installation of internal packages. Lightweight source collection remains available without `vue-tsc`, but checker-parity semantic dependency and reference resolution fail closed for unsupported tuples. Limina no longer exposes `config.imports.vue` or `VueImportParser`, and no longer directly declares, loads, or validates `@vue/compiler-sfc`. Vue checker and editor tooling own SFC structural diagnostics; Limina retains only provenance, resolution, and graph diagnostics.
