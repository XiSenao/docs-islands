...
description: Production guidance for using @docs-islands/logger in applications, libraries, framework hosts, tests, and bundler configs. Use when adding logger calls, choosing runtime visibility, configuring rules, enabling build-time pruning, integrating explicit scoped logging, reviewing package usage, or debugging why logs are shown, hidden, or not tree-shaken.
---

# @docs-islands/logger

Use this skill to make production-safe decisions for `@docs-islands/logger`.
...
1. Identify the config owner before adding code:
   ...
   - Framework or host integration: create an explicit scope with `@docs-islands/logger/core`.
